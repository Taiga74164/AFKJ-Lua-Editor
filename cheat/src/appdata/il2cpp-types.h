...
namespace app
{
	...
	enum class LuaTypes__Enum : int32_t
	{
		LUA_TNONE = -1,
		LUA_TNIL = 0x00000000,
		LUA_TBOOLEAN = 0x00000001,
		LUA_TLIGHTUSERDATA = 0x00000002,
		LUA_TNUMBER = 0x00000003,
		LUA_TSTRING = 0x00000004,
		LUA_TTABLE = 0x00000005,
		LUA_TFUNCTION = 0x00000006,
		LUA_TUSERDATA = 0x00000007,
		LUA_TTHREAD = 0x00000008,
	};

	struct __declspec(align(8)) LuaBaseRef__Fields
	{
		struct String* name;
		int32_t reference;
	...
	struct LuaBaseRef
	{
		struct LuaBaseRef__Class* klass;
		MonitorData* monitor;
		struct LuaBaseRef__Fields fields;
	};

	struct LuaFunction__Fields
	{
		struct LuaBaseRef__Fields _;
		int32_t oldTop;
		int32_t argCount;
		int32_t stackPos;
		struct Stack_1_LuaInterface_LuaFunction_FuncData_* stack;
	};

	struct LuaFunction
	{
		struct LuaFunction__Class* klass;
		MonitorData* monitor;
	...
	struct LuaClient__Fields
	{
		struct MonoBehaviour__Fields _;
		struct LuaState* luaState;
		struct LuaLooper* loop;
		struct LuaFunction* levelLoaded;
		bool beZbStart;
		struct LuaTable* profiler;
	};

	struct LuaClient
	{
		struct LuaClient__Class* klass;
		MonitorData* monitor;
		struct LuaClient__Fields fields;
	};

	struct LuaManager__Fields
	{
		struct LuaClient__Fields _;
		struct String* mainFunc;
		struct LuaFunction* applicationPause;
		struct LuaFunction* callLuaEvent;
		struct LuaFunction* onTriggerOrientationAspectEvent;
		struct SimpleScreenOrientationAspect* m_screenOrientationAspect;
	};
